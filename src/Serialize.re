...
let copy = (o_: 'a): 'a => {
  let o = o_->Obj.magic;
  isArray(o)
    ? Array.copy(o)->Obj.magic : Js.Obj.assign(Js.Obj.empty(), o)->Obj.magic;
};
let rec wrapTag = o_ => {
  let o = Obj.magic(o_);
  switch (Js.typeof(o)) {
...
let rec unwrapTag = o_ => {
  ...
  | "object" when o === Js.null => o
  | "object" =>
    switch (isArray(o) ? o[0] : None) {
    | Some(id) when id == undefinedID => Js.undefined->Obj.magic
    | Some(id) when id == tagID =>
      let tag = o->(Array.getExn(1));
      let innerObj = o->(Array.getExn(2));
